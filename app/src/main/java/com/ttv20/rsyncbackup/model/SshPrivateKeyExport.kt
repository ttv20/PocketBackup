...
@Serializable
data class EncryptedTailscaleStateExport(
    val version: Int = 1,
    val kdf: String = "PBKDF2WithHmacSHA256",
    val cipher: String = "AES-256-GCM",
    val iterations: Int,
...
@Serializable
data class TailscaleStateExportPayload(
    val stateArchiveBase64: String,
) {
    fun stateArchiveBytes(): ByteArray =
        Base64.getDecoder().decode(stateArchiveBase64)

    companion object {
        fun fromStateArchive(bytes: ByteArray): TailscaleStateExportPayload =
            TailscaleStateExportPayload(Base64.getEncoder().encodeToString(bytes))
    }
}
...
object TailscaleStateExportCrypto {
    private const val VERSION = 1
    private const val KDF_ALGORITHM = "PBKDF2WithHmacSHA256"
    private const val CIPHER_NAME = "AES-256-GCM"
    private const val CIPHER_TRANSFORMATION = "AES/GCM/NoPadding"
    ...
    private const val MIN_ITERATIONS = 100_000
    private const val MAX_ITERATIONS = 2_000_000
    private val aad = "pocketbackup-tailscale-state-export-v1".toByteArray(Charsets.UTF_8)

    fun encrypt(
        payload: TailscaleStateExportPayload,
        password: String,
        secureRandom: SecureRandom = SecureRandom(),
        iterations: Int = DEFAULT_ITERATIONS,
    ): EncryptedTailscaleStateExport {
        require(password.isNotBlank()) { "Tailscale state export password is required" }
        require(iterations in MIN_ITERATIONS..MAX_ITERATIONS) { "Unsupported Tailscale state export KDF cost" }
        ...
        cipher.updateAAD(aad)
        val plaintext = ExportCodec.json.encodeToString(
            TailscaleStateExportPayload.serializer(),
            payload,
        ).toByteArray(Charsets.UTF_8)
        return EncryptedTailscaleStateExport(
            version = VERSION,
            kdf = KDF_ALGORITHM,
            cipher = CIPHER_NAME,
            iterations = iterations,
    ...
    fun decrypt(export: EncryptedTailscaleStateExport, password: String): TailscaleStateExportPayload {
        require(password.isNotBlank()) { "Tailscale state export password is required" }
        require(export.version == VERSION) { "Unsupported Tailscale state export version" }
        require(export.kdf == KDF_ALGORITHM) { "Unsupported Tailscale state export KDF" }
        require(export.cipher == CIPHER_NAME) { "Unsupported Tailscale state export cipher" }
        require(export.iterations in MIN_ITERATIONS..MAX_ITERATIONS) { "Unsupported Tailscale state export KDF cost" }
        ...
        return try {
            ...
            cipher.updateAAD(aad)
            val plaintext = cipher.doFinal(ciphertext).toString(Charsets.UTF_8)
            ExportCodec.json.decodeFromString(TailscaleStateExportPayload.serializer(), plaintext)
        } catch (error: Exception) {
            throw IllegalArgumentException("Tailscale state export password is incorrect or the export is corrupted", error)
        }
    }
